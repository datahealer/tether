...
export const acceptInvite = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Optimized: Fetch both users in parallel with lean() and select only needed fields
    const [inviter, accepter] = await Promise.all([
      User.findById(invite.inviterId).select('coupleId subscribed').lean(),
      User.findById(userId).select('coupleId subscribed').lean(),
    ]);
