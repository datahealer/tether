...
export default function CategoryPacksScreen() {
  ...
  // Simulating waiting tether state
  const [hasWaitingTether, setHasWaitingTether] = useState(true);
  ...
  const handleCardPress = async (card: CarouselCard, index: number) => {
    if (card.isLocked) {
      ...
      Alert.alert(
        'Pack Locked',
        'Complete more questions in previous packs to unlock this category!',
